...
import { FileText, Lock, User, Eye, EyeOff, Loader2 } from "lucide-react";
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const { data: authData, error } = await supabase.auth.signInWithPassword({
        email: `${numeroDocumento}@locador.local`,
        password: password,
      });

      if (error) throw error;

      const { data: locador, error: locadorError } = await supabase
        .from("locadores")
        .select("id")
        ...
        >
          ...
          <form onSubmit={handleLogin} className="space-y-5">
            <div className="space-y-2">
              <Label htmlFor="documento" className="text-sm font-medium text-foreground/80">
                Número de Documento (DNI o CE)
              </Label>
