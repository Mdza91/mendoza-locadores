...
import { UserPlus, Save, User, Building2, CreditCard, CalendarDays } from "lucide-react";
import { locadorSchemaWithRefinement } from "@/lib/validationSchemas";
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
export const LocadorFormDialog = ({ open, onOpenChange, locadorId }: LocadorFormDialogProps) => {
  const queryClient = useQueryClient();
  const isEditing = !!locadorId;
  ...
  const loadLocador = async () => {
    const { data, error } = await supabase.from("locadores").select("*").eq("id", locadorId).single();
  ...
  const saveMutation = useMutation({
    mutationFn: async (data: any) => {
      if (isEditing) { const { error } = await supabase.from("locadores").update(data).eq("id", locadorId); if (error) throw error; }
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["locador", locadorId] });
      toast.success(isEditing ? "Locador actualizado exitosamente" : "Locador creado exitosamente");
      onOpenChange(false);
    },
    onError: (error: any) => {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl max-h-[92vh] p-0 rounded-2xl border-border/50 shadow-xl overflow-hidden">
        <DialogHeader className="px-6 pt-6 pb-4 bg-muted/20 border-b border-border/30">
          <div className="flex items-center gap-3">
            <div className="h-10 w-10 rounded-xl bg-primary/10 flex items-center justify-center">
              {isEditing ? <Save className="h-5 w-5 text-primary" /> : <UserPlus className="h-5 w-5 text-primary" />}
            </div>
            <div>
              <DialogTitle className="text-xl font-bold">{isEditing ? "Editar Locador" : "Nuevo Locador"}</DialogTitle>
              <p className="text-sm text-muted-foreground mt-0.5">{isEditing ? "Actualice la información del locador" : "Complete el formulario para registrar un nuevo locador"}</p>
            </div>
          </div>
        </DialogHeader>
        ...
        <ScrollArea className="max-h-[calc(92vh-10rem)]">
          <form onSubmit={handleSubmit} className="p-6 space-y-5">
            ...
            {/* Estado (solo edición) */}
            {isEditing && (
              <div className="rounded-2xl border border-border/40 overflow-hidden animate-fade-in">
                <div className="bg-muted/30 px-4 py-3 border-b border-border/30 flex items-center gap-2">
