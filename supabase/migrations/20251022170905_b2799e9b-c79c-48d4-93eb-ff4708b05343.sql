...
DROP TYPE IF EXISTS public.document_stage CASCADE;
DROP TYPE IF EXISTS public.universal_document_type CASCADE;
...
CREATE TABLE IF NOT EXISTS public.locador_periodos (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  locador_id UUID NOT NULL REFERENCES public.locadores(id) ON DELETE CASCADE,
  etapa document_stage NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.documentos_generales (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE IF NOT EXISTS public.planilla_locadores (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
-- Insertar unidades por defecto (solo si no existen)
...
-- Políticas de storage para documentos (eliminar si existen y recrear)
...
-- Políticas RLS (eliminar si existen y recrear)
...
DROP POLICY IF EXISTS "Usuarios autenticados tienen acceso completo a planilla_locadores" ON public.planilla_locadores;
...
DROP TRIGGER IF EXISTS update_denominaciones_updated_at ON public.denominaciones;
DROP TRIGGER IF EXISTS update_locadores_updated_at ON public.locadores;
DROP TRIGGER IF EXISTS update_locador_periodos_updated_at ON public.locador_periodos;
DROP TRIGGER IF EXISTS update_documentos_generales_updated_at ON public.documentos_generales;
DROP TRIGGER IF EXISTS update_planillas_updated_at ON public.planillas;
DROP TRIGGER IF EXISTS update_config_notificaciones_updated_at ON public.config_notificaciones;
...
CREATE INDEX IF NOT EXISTS idx_locadores_apellidos ON public.locadores(apellidos);
CREATE INDEX IF NOT EXISTS idx_locadores_unidad ON public.locadores(unidad_id);
CREATE INDEX IF NOT EXISTS idx_locadores_denominacion ON public.locadores(denominacion_id);
CREATE INDEX IF NOT EXISTS idx_locadores_activo ON public.locadores(activo);
CREATE INDEX IF NOT EXISTS idx_documentos_locador ON public.documentos(locador_id);
CREATE INDEX IF NOT EXISTS idx_documentos_vencimiento ON public.documentos(fecha_vencimiento);
CREATE INDEX IF NOT EXISTS idx_planilla_locadores_planilla ON public.planilla_locadores(planilla_id);
CREATE INDEX IF NOT EXISTS idx_planilla_locadores_locador ON public.planilla_locadores(locador_id);